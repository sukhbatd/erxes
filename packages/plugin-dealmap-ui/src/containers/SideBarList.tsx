...
type FinalProps = {
  listDealmapTypeQuery: TypeQueryResponse;
} & Props &
  RemoveTypeMutationResponse &
  EditTypeMutationResponse;

const TypesListContainer = (props: FinalProps) => {
  const { listDealmapTypeQuery, typesEdit, typesRemove, history } = props;

  if (listDealmapTypeQuery.loading) {
    return <Spinner />;
  }
  ...
  const renderButton = ({
    ...
    return (
      <ButtonMutate
        ...
        refetchQueries={['listDealmapTypeQuery']}
      />
    );
  };
  ...
  const updatedProps = {
    ...props,
    types: listDealmapTypeQuery.dealmapTypes || [],
    loading: listDealmapTypeQuery.loading,
    remove,
    renderButton
  };
...
export default withProps<Props>(
  compose(
    graphql(gql(queries.listDealmapTypes), {
      name: 'listDealmapTypeQuery',
      options: () => ({
